...
import {setLogger as zhcSetLogger, access} from "zigbee-herdsman-converters";
        ...
        for (const endpoint of device.zh.endpoints) {
            ...
            for (const [clusterName, cluster] of Object.entries(endpoint.clusters)) {
                ...
                const converters = device.definition.fromZigbee.filter(
                    (c) => c.cluster === clusterName && (c.type === "attributeReport" || Array.isArray(c.type) && c.type.includes("attributeReport")),
                );
